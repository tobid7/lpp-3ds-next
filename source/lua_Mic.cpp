...
u32 sampleSize(MICU_SampleRate smplrt) {
  if (smplrt == MICU_SAMPLE_RATE_8180)
    return 8180;
  else if (smplrt == MICU_SAMPLE_RATE_10910)
    return 10910;
  else if (smplrt == MICU_SAMPLE_RATE_16360)
    return 16360;
  else
    return 32730;
}
...
static int lua_resumemic(lua_State *L) {
  ...
  u32 offset;
  if (!isSampling)
    MICU_StartSampling(MICU_ENCODING_PCM16_SIGNED, srate, cur_offset,
                       micsize - cur_offset, false);
  return 0;
}

// Register our Mic Functions
static const luaL_Reg Mic_DSP_functions[] = {
    {"start", lua_regsound},   {"isRecording", lua_micplaying},
    {"stop", lua_stoprec_DSP}, {"pause", lua_pausemic},
    {"resume", lua_resumemic}, {0, 0}};

static const luaL_Reg Mic_CSND_functions[] = {
    {"start", lua_regsound},    {"isRecording", lua_micplaying},
    {"stop", lua_stoprec_CSND}, {"pause", lua_pausemic},
    {"resume", lua_resumemic},  {0, 0}};

void luaMic_init(lua_State *L) {
  lua_newtable(L);
  if (csndAccess)
    luaL_setfuncs(L, Mic_CSND_functions, 0);
  else
    luaL_setfuncs(L, Mic_DSP_functions, 0);
  lua_setglobal(L, "Mic");
}