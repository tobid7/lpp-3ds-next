...
static int lua_lcdon(lua_State *L) {
  ...
  u32 flag;
  if (screen == 0)
    flag = GSPLCD_SCREEN_TOP;
  else if (screen == 1)
    flag = GSPLCD_SCREEN_BOTTOM;
  else
    return luaL_error(L, "wrong parameter.");
  if (gspLcdInit() == 0) {
    GSPLCD_PowerOnBacklight(flag);
    gspLcdExit();
    if (flag == GSPLCD_SCREEN_TOP)
      isTopLCDOn = true;
    else
      isBottomLCDOn = true;
  }
...
static const luaL_Reg Controls_functions[] = {{"read", lua_readC},
                                              {"check", lua_check},
                                              {"enableGyro", lua_eg},
                                              {"enableAccel", lua_ea},
                                              {"disableGyro", lua_dg},
