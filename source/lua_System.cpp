...
static int lua_openfile(lua_State *L)
{
	...
	else if (extdata)
	{
		...
		FS_Archive main_extdata_archive;
		Result ret = FSUSER_OpenArchive(&main_extdata_archive, atype, (FS_Path){PATH_BINARY, 0xC, (u8 *)main_extdata_archive_lowpathdata});
#ifndef SKIP_ERROR_HANDLING
		if (ret != 0)
	...
	else
	{
		...
		{
		case 0:
			ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_READ, 0x00000000);
			break;
		case 1:
			ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_WRITE, 0x00000000);
			break;
		case 2:
			ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_CREATE | FS_OPEN_WRITE, 0x00000000);
			break;
		}
#ifndef SKIP_ERROR_HANDLING
		if (ret)
...
static int lua_screenshot(lua_State *L)
{
	...
	{ // BMP Format
		...
		if (ret)
			return luaL_error(L, "error opening file"); // Sometimes this gives false positives
		u32 bytesWritten;
...
#ifndef SKIP_ERROR_HANDLING
	if (!path)
		return luaL_error(L, "System.currentDirectory(file) takes a filename as a string argument.");
#endif
	strcpy(cur_dir, path);
	return 0;
}
...
static int lua_createdir(lua_State *L)
{
	...
	if (argc == 2)
	{
		...
		u32 main_extdata_archive_lowpathdata[3] = {mtype, archive_id, 0};
		FS_Archive archive2;
		Result ret = FSUSER_OpenArchive(&archive2, atype, (FS_Path){PATH_BINARY, 0xC, (u8 *)main_extdata_archive_lowpathdata});
#ifndef SKIP_ERROR_HANDLING
		if (ret != 0)
			return luaL_error(L, "cannot access extdata archive");
#endif
	...
	{
		FS_Archive archive2;
		FSUSER_OpenArchive(&archive2, ARCHIVE_SDMC, (FS_Path){PATH_EMPTY, 1, (u8 *)""});
		FS_Path filePath = fsMakePath(PATH_ASCII, path);
		FSUSER_CreateDirectory(archive2, filePath, FS_ATTRIBUTE_DIRECTORY);
		FSUSER_CloseArchive(archive2);
	}
...
static int lua_readsmdh(lua_State *L)
{
	...
	else
	{
		...
		Result ret = FSUSER_OpenFileDirectly(&tmp.handle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_READ, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
		if (ret)
			return luaL_error(L, "file doesn't exist.");
#endif
...
#endif
	// const char *file = luaL_checkstring(L, 1);
	std::cout << "the function " << stringify(lua_launch) << " is Deprecated\nand not Supportet by Current\nninjahax and luma3ds!" << std::endl;
...
static int lua_listExtdata(lua_State *L)
{
	...
	for (i = 0xE0000000; i < 0xE0000100; ++i)
	{
		...
		FS_Archive extdata_archive;
		Result ret = FSUSER_OpenArchive(&extdata_archive, ARCHIVE_SHARED_EXTDATA, (FS_Path){PATH_BINARY, 0xC, (u8 *)extdata_archive_lowpathdata});
		if (ret != 0)
			continue;
		FSUSER_OpenDirectory(&extdata_dir, extdata_archive, fsMakePath(PATH_ASCII, "/"));
		FS_dirent entry;
...
static int lua_listExtdataDir(lua_State *L)
{
	...
	FS_Archive extdata_archive;
	Result ret = FSUSER_OpenArchive(&extdata_archive, atype, (FS_Path){PATH_BINARY, 0xC, (u8 *)extdata_archive_lowpathdata});
#ifndef SKIP_ERROR_HANDLING
	if (ret != 0)
		return luaL_error(L, "cannot access extdata archive");
#endif
...
static int lua_ciainfo(lua_State *L)
{
	...
	FSUSER_OpenArchive(&sdmcArchive, ARCHIVE_SDMC, (FS_Path){PATH_EMPTY, 1, (u8 *)""});
	FS_Path filePath = fsMakePath(PATH_ASCII, file);
	Result ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_READ, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
	if (ret)
		return luaL_error(L, "file doesn't exist.");
#endif
...
static int lua_brahmaloader(lua_State *L)
{
	...
	if (argc != 2)
		return luaL_error(L, "wrong number of arguments.");
#endif
	std::cout << "The function " << stringify(lua_brahmaloader) << " is completly \ndeprecated and will never\nbe supportet again!" << std::endl;
	return 0;
}
...
static int lua_syscall1(lua_State *L)
{
	...
	if (argc != 0)
		return luaL_error(L, "wrong number of arguments.");
#endif
	std::cout << "The function " << stringify(lua_syscall1) << " is completly \ndeprecated and will never\nbe supportet again!" << std::endl;
	return 0;
}
...
static int lua_appstatus(lua_State *L)
{
	...
	std::cout << "The function " << stringify(lua_appstatus) << " is completly \ndeprecated and will never\nbe supportet again!" << std::endl;
	return 0;
}
	...
	if (argc == 4)
	{
		...
		if (isFile)
		{
			...
			else
			{
				...
				FS_Archive script;
				FSUSER_OpenArchive(&script, ARCHIVE_SDMC, (FS_Path){PATH_EMPTY, 1, (u8 *)""});
				Result ret = FSUSER_OpenFileDirectly(&tmp.handle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_READ, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
				if (ret)
					return luaL_error(L, "file doesn't exist.");
		...
		if (!processed)
		{
			...
			if (moltiplier == 4)
			{ // 32bpp image - Need to delete alpha channel
				u8 *tmp = flip_pixels;
...
static int lua_dup(lua_State *L)
{ // TODO: Add Music and wav struct support
	int argc = lua_gettop(L);
#ifndef SKIP_ERROR_HANDLING
	if (argc != 1)
		return luaL_error(L, "wrong number of arguments.");
