...
static int lua_dofile(lua_State *L) {
  ...
  lua_settop(L, 1);
  if (luaL_loadbuffer(L, (const char *)buffer, strlen((const char *)buffer),
                      NULL) != LUA_OK)
    return lua_error(L);
...
static int lua_openfile(lua_State *L) {
  ...
  if (argc >= 3) {
    ...
    extdata = true;
    if (argc == 4)
      filesize = luaL_checkinteger(L, 4);
  }
  ...
  if (strncmp("romfs:/", file_tbo, 7) == 0) {
    ...
    FS_Archive main_extdata_archive;
    Result ret = FSUSER_OpenArchive(
        &main_extdata_archive, atype,
        (FS_Path){PATH_BINARY, 0xC, (u8 *)main_extdata_archive_lowpathdata});
#ifndef SKIP_ERROR_HANDLING
    if (ret != 0)
      return luaL_error(L, "cannot access extdata archive");
#endif
    switch (type) {
    case 0:
      ret = FSUSER_OpenFile(&fileHandle, main_extdata_archive, filePath,
                            FS_OPEN_READ, 0);
      break;
    case 1:
      ret = FSUSER_OpenFile(&fileHandle, main_extdata_archive, filePath,
...
#ifndef SKIP_ERROR_HANDLING
      if (ret)
        return luaL_error(L, "error creating the file.");
#endif
      ret = FSUSER_OpenFile(&fileHandle, main_extdata_archive, filePath,
                            FS_OPEN_WRITE, 0);
      break;
    ...
    switch (type) {
    case 0:
      ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath,
                                    filePath, FS_OPEN_READ, 0x00000000);
      break;
    case 1:
      ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath,
                                    filePath, FS_OPEN_WRITE, 0x00000000);
      break;
    case 2:
      ret =
...
#endif
  if (CIA_MODE)
    lua_pushinteger(L, 1); // cia
  else
    lua_pushinteger(L, 2); // Ninjahax 2
  return 1;
}
...
static int lua_screenshot(lua_State *L) {
  ...
  if (!isJPG) { // BMP Format
    ...
    *(u32 *)&tempbuf[0x22] = 576000;
    ...
    FSFILE_Write(fileHandle, &bytesWritten, 0, (u32 *)tempbuf, 0x36 + 576000,
                 0x10001);
...
static int lua_writefile(lua_State *L) {
  ...
  if (file->isRomfs)
    luaL_error(L, "romFs partition is read-only");
  else {
    u32 bytesWritten;
    FSFILE_Write(file->handle, &bytesWritten, init, text, size, FS_WRITE_FLUSH);
    // if(ret || size!=bytesWritten) return luaL_error(L, "error writing file");
    // // Sometimes this gives false positives
  }
  return 0;
}
...
static int lua_setCurrentDirectory(lua_State *L) {
  ...
  if (!path)
    return luaL_error(
        L,
        "System.currentDirectory(file) takes a filename as a string argument.");
#endif
  strcpy(cur_dir, path);
  return 0;
}
...
#ifndef SKIP_ERROR_HANDLING
  return luaL_error(
      L, "System.currentDirectory([file]) takes zero or one argument");
#endif
}
...
static int lua_createdir(lua_State *L) {
  ...
  if (argc == 2) {
    ...
    FS_Archive archive2;
    FSUSER_OpenArchive(&archive2, ARCHIVE_SDMC,
                       (FS_Path){PATH_EMPTY, 1, (u8 *)""});
    FS_Path filePath = fsMakePath(PATH_ASCII, path);
    FSUSER_CreateDirectory(archive2, filePath, FS_ATTRIBUTE_DIRECTORY);
    FSUSER_CloseArchive(archive2);
...
static int lua_readsmdh(lua_State *L) {
  ...
  if (strncmp("romfs:/", file, 7) == 0) {
    ...
    Result ret = FSUSER_OpenFileDirectly(&tmp.handle, ARCHIVE_SDMC, filePath,
                                         filePath, FS_OPEN_READ, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
    if (ret)
      return luaL_error(L, "file doesn't exist.");
  ...
  while (tile_number < 37) {
    while (i < (tile_size)) {
      putPixel565(
          bitmap->pixels, tile_x[i - ((tile_number - 1) << 6)] + extra_x,
          tile_y[i - ((tile_number - 1) << 6)] + extra_y, icon_buffer[i]);
      putPixel565(
          bitmap->pixels, 4 + tile_x[i - ((tile_number - 1) << 6)] + extra_x,
          tile_y[i - ((tile_number - 1) << 6)] + extra_y, icon_buffer[i + 16]);
      putPixel565(bitmap->pixels,
                  tile_x[i - ((tile_number - 1) << 6)] + extra_x,
                  4 + tile_y[i - ((tile_number - 1) << 6)] + extra_y,
                  icon_buffer[i + 32]);
      putPixel565(bitmap->pixels,
                  4 + tile_x[i - ((tile_number - 1) << 6)] + extra_x,
                  4 + tile_y[i - ((tile_number - 1) << 6)] + extra_y,
                  icon_buffer[i + 48]);
      i++;
    }
...
// void (*callBootloader_2x)(Handle file, u32 *argbuf, u32 arglength) = (void
// *)0x00100000; static void launchFile_2x(void) { callBootloader_2x(hbHandle,
// argbuffer, argbuffer_length); }
#include <iostream>
...
static int lua_launch(lua_State *L) {
  ...
  // const char *file = luaL_checkstring(L, 1);
  std::cout
      << "the function " << stringify(lua_launch)
      << " is Deprecated\nand not Supportet by Current\nninjahax and luma3ds!"
      << std::endl;
  return 0; // NOTE: This is a fake error
}
...
static int lua_listExtdata(lua_State *L) {
  ...
  for (i = 0; i < 0x2000; ++i) {
    ...
    Result ret = FSUSER_OpenArchive(
        &extdata_archive, ARCHIVE_EXTDATA,
        (FS_Path){PATH_BINARY, 0xC, (u8 *)extdata_archive_lowpathdata});
    if (ret != 0)
      continue;
    FSUSER_OpenDirectory(&extdata_dir, extdata_archive,
  ...
  for (i = 0xE0000000; i < 0xE0000100; ++i) {
    ...
    FS_Archive extdata_archive;
    Result ret = FSUSER_OpenArchive(
        &extdata_archive, ARCHIVE_SHARED_EXTDATA,
        (FS_Path){PATH_BINARY, 0xC, (u8 *)extdata_archive_lowpathdata});
    if (ret != 0)
      continue;
    FSUSER_OpenDirectory(&extdata_dir, extdata_archive,
...
static int lua_listCia(lua_State *L) {
  ...
  while (i <= cia_nums) {
    ...
    lua_settable(L, -3);
    u64 id = TitleIDs[i - 1].uniqueid | ((u64)TitleIDs[i - 1].category << 32) |
             ((u64)TitleIDs[i - 1].platform << 48);
    char product_id[16];
    ...
    if (((TitleIDs[i - 1].category) & 0x8000) == 0x8000)
      lua_pushinteger(L, 4);
    else if (((TitleIDs[i - 1].category) & 0x10) == 0x10)
      lua_pushinteger(L, 1);
    else if (((TitleIDs[i - 1].category) & 0x6) == 0x6)
      lua_pushinteger(L, 3);
    else if (((TitleIDs[i - 1].category) & 0x2) == 0x2)
      lua_pushinteger(L, 2);
    else
      lua_pushnumber(L, 0);
    lua_settable(L, -3);
...
static int lua_uninstallCia(lua_State *L) {
  ...
  u64 id = TitleIDs[delete_id - 1].uniqueid |
           ...
           ((u64)TitleIDs[delete_id - 1].platform << 48);
...
static int lua_ciainfo(lua_State *L) {
  ...
  FSUSER_OpenArchive(&sdmcArchive, ARCHIVE_SDMC,
                     (FS_Path){PATH_EMPTY, 1, (u8 *)""});
  FS_Path filePath = fsMakePath(PATH_ASCII, file);
  Result ret = FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath,
                                       filePath, FS_OPEN_READ, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
  if (ret)
    return luaL_error(L, "file doesn't exist.");
...
static int lua_ZipExtract(lua_State *L) {
  ...
  chdir(tmpPath2);
  if (strncmp("romfs:/", FileToExtract, 7) == 0)
    strcpy(tmpFile2, FileToExtract);
  else {
  ...
  if (file == NULL)
    ...
    Result ret =
        FSUSER_OpenFileDirectly(&fileHandle, ARCHIVE_SDMC, filePath, filePath,
                                FS_OPEN_CREATE | FS_OPEN_WRITE, 0x00000000);
    u32 bytesWritten;
    ret = FSFILE_Write(fileHandle, &bytesWritten, 0, file->data, file->size,
                       FS_WRITE_FLUSH);
    FSFILE_Close(fileHandle);
    svcCloseHandle(fileHandle);
    ZipFileFree(file);
...
static int lua_brahmaloader(lua_State *L) {
  ...
  if (argc != 2)
    return luaL_error(L, "wrong number of arguments.");
#endif
  std::cout << "The function " << stringify(lua_brahmaloader)
...
static int lua_syscall1(lua_State *L) {
  ...
  if (argc != 0)
    return luaL_error(L, "wrong number of arguments.");
#endif
  std::cout << "The function " << stringify(lua_syscall1)
            << " is completly \ndeprecated and will never\nbe supportet again!"
            << std::endl;
  return 0;
}
...
static int lua_appstatus(lua_State *L) {
  ...
  std::cout << "The function " << stringify(lua_appstatus)
            << " is completly \ndeprecated and will never\nbe supportet again!"
            << std::endl;
  return 0;
}
...
static int lua_freespace(lua_State *L) {
  ...
  lua_pushnumber(L,
                 (u64)((u64)sdmcData.freeClusters * (u64)sdmcData.clusterSize));
  return 1;
}
...
static int lua_launchCia(lua_State *L) {
  ...
  APT_PrepareToDoApplicationJump(0, id, MEDIATYPE_GAME_CARD);
  APT_DoApplicationJump(param, sizeof(param), hmac);
  for (;;) {
  }
  return 0;
...
static int lua_getdate(lua_State *L) {
  ...
  while (time > control) {
    ...
    if ((month == 11) || (month == 4) || (month == 6) || (month == 9))
      control = month_seconds[0];
    else if (month == 2) {
      if (extended)
        control = month_seconds[3];
      else
        control = month_seconds[1];
    } else
      control = month_seconds[2];
  }
  day = day + time / 86400;
  ...
  if (argc == 4) {
    ...
    if (isFile) {
      ...
      if (strncmp("romfs:/", filename, 7) == 0) {
        ...
        FS_Archive script;
        FSUSER_OpenArchive(&script, ARCHIVE_SDMC,
                           (FS_Path){PATH_EMPTY, 1, (u8 *)""});
        Result ret =
            FSUSER_OpenFileDirectly(&tmp.handle, ARCHIVE_SDMC, filePath,
                                    filePath, FS_OPEN_READ, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
        if (ret)
          return luaL_error(L, "file doesn't exist.");
...
static int lua_getnews(lua_State *L) {
  ...
  newsInit();
  NEWS_GetNotificationMessage(id, message, &size);
...
static int lua_dup(lua_State *L) { // TODO: Add Music and wav struct support
  ...
  if (argc != 1)
    return luaL_error(L, "wrong number of arguments.");
#endif
  Music *src = (Music *)luaL_checkinteger(
  ...
  if (src->magic == 0x4C544D52) { // Timer
    ...
    *dst->material = {{
        {{0.0f, src2->material->r[0].c[1], src2->material->r[0].c[2],
          src2->material->r[0].c[3]}}, // Ambient
        {{0.0f, src2->material->r[1].c[1], src2->material->r[1].c[2],
          src2->material->r[1].c[3]}}, // Diffuse
        {{0.0f, src2->material->r[2].c[1], src2->material->r[2].c[2],
          src2->material->r[2].c[3]}},                   // Specular
    ...
    return luaL_error(L, "unsupported memory block input.");
