...
static int lua_caminit(lua_State *L) {
  ...
  if (is3D)
    cam_buf = (u8 *)malloc(SCREEN_SIZE << 1);
  else
    cam_buf = (u8 *)malloc(SCREEN_SIZE);
  return 0;
}
...
static int lua_camoutput(lua_State *L) {
  ...
  CAMU_SetReceiving(&camReceiveEvent, cam_buf, PORT_CAM1, SCREEN_SIZE,
                    (s16)bufSize);
  if (is3D)
...
static int lua_camshot(lua_State *L) {
  ...
  Handle camReceiveEvent = 0;
  CAMU_SetReceiving(&camReceiveEvent, framebuf, PORT_CAM1, framebuf_size,
                    (s16)bufSize);
  svcWaitSynchronization(camReceiveEvent, WAIT_TIMEOUT);
...
static const luaL_Reg Camera_functions[] = {
    {"init", lua_caminit},      {"getOutput", lua_camoutput},
    {"takePhoto", lua_camshot}, {"takeImage", lua_camimage},
    {"term", lua_camexit},      {0, 0}};
