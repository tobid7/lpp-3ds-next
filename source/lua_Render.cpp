...
static int lua_loadobj(lua_State *L) {
...
#ifndef SKIP_ERROR_HANDLING
  if (ambient->magic != 0xC0C0C0C0)
    return luaL_error(L, "attempt to access wrong memory block type");
  if (diffuse->magic != 0xC0C0C0C0)
    return luaL_error(L, "attempt to access wrong memory block type");
  if (specular->magic != 0xC0C0C0C0)
    return luaL_error(L, "attempt to access wrong memory block type");
#endif
  ...
  for (j = 0; j < bitmap->height; j++) {
    for (i = 0; i < bitmap->width; i++) {
      ...
      u32 v =
          ((u32 *)bitmap->pixels)[i + (bitmap->height - 1 - j) * bitmap->width];
...
static int lua_init(lua_State *L) {
  ...
  targets[0] =
      C3D_RenderTargetCreate(h, w, GPU_RB_RGBA8, GPU_RB_DEPTH24_STENCIL8);
  C3D_RenderTargetClear(targets[0], C3D_CLEAR_ALL, CLEAR_COLOR, 0);
  C3D_RenderTargetSetOutput(targets[0], GFX_TOP, GFX_LEFT,
                            DISPLAY_TRANSFER_FLAGS);
  targets[1] =
      C3D_RenderTargetCreate(h, w, GPU_RB_RGBA8, GPU_RB_DEPTH24_STENCIL8);
  C3D_RenderTargetClear(targets[1], C3D_CLEAR_ALL, CLEAR_COLOR, 0);
  C3D_RenderTargetSetOutput(targets[1], GFX_TOP, GFX_RIGHT,
                            DISPLAY_TRANSFER_FLAGS);
  targets[2] =
      C3D_RenderTargetCreate(h, w, GPU_RB_RGBA8, GPU_RB_DEPTH24_STENCIL8);
  C3D_RenderTargetClear(targets[2], C3D_CLEAR_ALL, CLEAR_COLOR, 0);
  C3D_RenderTargetSetOutput(targets[2], GFX_BOTTOM, GFX_LEFT,
                            DISPLAY_TRANSFER_FLAGS);
  ...
  uLoc_lightHalfVec =
      shaderInstanceGetUniformLocation(program.vertexShader, "lightHalfVec");
  uLoc_lightClr =
      shaderInstanceGetUniformLocation(program.vertexShader, "lightClr");
  uLoc_material =
      shaderInstanceGetUniformLocation(program.vertexShader, "material");
...
static int lua_initblend(lua_State *L) {
  ...
  C3D_FVUnifSet(GPU_VERTEX_SHADER, uLoc_lightVec, light_x, light_y, light_z,
                0.0f);
  C3D_FVUnifSet(GPU_VERTEX_SHADER, uLoc_lightHalfVec, light_x, light_y, light_z,
                0.0f);
  C3D_FVUnifSet(GPU_VERTEX_SHADER, uLoc_lightClr, light_r, light_g, light_b,
                light_a);
...
static int lua_blend(lua_State *L) {
  ...
  Mtx_Translate(&modelView, x, y, z, false);
...
static const luaL_Reg Render_functions[] = {{"createVertex", lua_newVertex},
