...
int ftp_openCommandChannel() {
  if (listenfd < 0) {
    ...
    setsockopt(listenfd, SOL_SOCKET, SO_RCVBUF, &sock_buffersize,
  ...
  if (cmd_sock >= 0) {
    // closesocket(listenfd);
    // listenfd=-1;
    setsockopt(cmd_sock, SOL_SOCKET, SO_RCVBUF, &sock_buffersize,
               sizeof(sock_buffersize));
...
int ftp_openDataChannel() {
  ...
  setsockopt(ret, SOL_SOCKET, SO_RCVBUF, &sock_buffersize,
             sizeof(sock_buffersize));
  closesocket(listenfd);
...
int ftp_processCommand(int s, char *data) {
  ...
  int i;
  for (i = 0; i < ftp_cmd_num; i++)
    if (!strcmp(cmd, ftp_cmd[i].name)) {
