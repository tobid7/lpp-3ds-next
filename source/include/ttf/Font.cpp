...
void Font::setScale(float scale) { m_scale = scale; }

void Font::setSize(int pixels) {
  m_scale = stbtt_ScaleForPixelHeight(&m_info, pixels);
}

void Font::drawStringUnicode(int x, int y, const std::wstring &str, Color color,
                             bool top_screen, bool side, int max_width) {
  if (!isLoaded()) {
    return;
  }
  ...
  for (unsigned int i = 0; i < str.length(); i++) {
    ...
    stbtt_GetCodepointBitmapBox(&m_info, str[i], m_scale, m_scale, &bx, &by,
                                &bw, &bh);
    ...
    stbtt_MakeCodepointBitmap(&m_info, &char_raster[0], width, height, width,
                              m_scale, m_scale, str[i]);
    ...
    for (int ix = 0; ix < width; ix++) {
      for (int iy = 0; iy < height; iy++) {
        if (char_raster[ix + iy * width] != 0)
          char_render.putPixel(
              ix, iy,
              Color(color.r, color.g, color.b, char_raster[ix + iy * width]));
      }
    }
...
void Font::drawString(int x, int y, const std::string &str, Color color,
                      bool top_screen, bool side, int max_width) {
  if (str.length() > 0) {
...
void Font::measureText(const std::wstring str, int &width, int &height,
                       int max_width) {
  if (!isLoaded()) {
    return;
  }
...
void Font::drawStringToBuffer(int x, int y, const std::wstring &str,
                              Color color, unsigned char *buffer,
                              int buffer_width, int buffer_height,
                              int bitsperpixel, int max_width) {
  std::vector<unsigned char> char_raster;
  int bx, by, bw, bh;
  int ascent, descent, lineGap;
  int sx = 0, sy = 0;
  ...
  for (unsigned int i = 0; i < str.length(); i++) {
    ...
    for (int ix = 0; ix < char_width; ix++) {
      for (int iy = 0; iy < char_height; iy++) {
        ...
        if (char_raster[ix + iy * char_width] != 0 && xpos < buffer_width &&
            ypos < buffer_height) {
          ...
          if (bitsperpixel == 24) {
            ...
            unsigned char r =
                (unsigned char)((alpha * color.r + inv_alpha * bg_r) >> 8);
            unsigned char g =
                (unsigned char)((alpha * color.g + inv_alpha * bg_g) >> 8);
            unsigned char b =
                (unsigned char)((alpha * color.b + inv_alpha * bg_b) >> 8);
...
bool Font::isLoaded() { return m_info.data != 0; }
