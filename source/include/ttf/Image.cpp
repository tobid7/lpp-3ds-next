...
Image::Image(const std::string &filename) { loadFromFile(filename); }
...
bool Image::isLoaded() const {
  return m_width > 0 && m_height > 0 && m_pixels.size() > 0;
}
...
void Image::draw(int x, int y, bool top_screen, bool side) {
  ...
  u8 *frameBuffer = gfxGetFramebuffer(top_screen ? GFX_TOP : GFX_BOTTOM,
                                      side ? GFX_LEFT : GFX_RIGHT,
  ...
  for (int ix = 0; ix < getWidth(); ix++) {
    for (int iy = 0; iy < getHeight(); iy++) {
      ...
      unsigned char res_r =
          (unsigned char)((alpha * fg_r + inv_alpha * bg_r) >> 8);
      unsigned char res_g =
          (unsigned char)((alpha * fg_g + inv_alpha * bg_g) >> 8);
      unsigned char res_b =
          (unsigned char)((alpha * fg_b + inv_alpha * bg_b) >> 8);
