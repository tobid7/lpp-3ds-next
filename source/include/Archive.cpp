...
static int ZipUpdateKeys(unsigned long *pkeys, const unsigned long *crc32tab,
                         int c) {
...
static void ZipInitKeys(const char *passwd, unsigned long *pkeys,
                        const unsigned long *crc32tab) {
  *(pkeys + 0) = 305419896L;
  *(pkeys + 1) = 591751049L;
  *(pkeys + 2) = 878082192L;
...
static unsigned long ZipLocateCentralDir(FILE *file) {
  ...
  while (ubackread < umaxback) {
    ...
    ureadsize = ((_ZIP_BUF_READ_COMMENT + 4) < (usizefile - ureadpos))
...
static int ZipGotoFirstFile(Zip *file) {
  ...
  s->numfile = 0;
  err = ZitZipFileInfoInternal(file, &s->currentfileinfo,
                               &s->currentfileinfointernal, NULL, 0, NULL, 0,
                               NULL, 0);
...
static int ZitCurrentFileInfo(Zip *file, zipFileInfo *pfileinfo, char *filename,
                              unsigned long filenamebuffersize,
                              void *extrafield,
                              unsigned long extrafieldbuffersize, char *comment,
                              unsigned long commentbuffersize) {
  return ZitZipFileInfoInternal(
      file, pfileinfo, NULL, filename, filenamebuffersize, extrafield,
...
static int ZipGotoNextFile(Zip *file) {
  ...
  s->posincentraldir +=
      ...
      s->currentfileinfo.fileextrasize + s->currentfileinfo.filecommentsize;
  s->numfile++;
...
ZipCheckCurrentFileCoherencyHeader(_zip *s, unsigned int *pisizevar,
  ...
  *psizestaticextrafield = 0;
  ...
  if (fseek(s->file,
            s->currentfileinfointernal.currentfileoffset + s->bytebeforezip,
            SEEK_SET) != 0)
    return _ZIP_ERRNO;
  ...
  if ((err == _ZIP_OK) && (s->currentfileinfo.compressionmethod != 0) &&
  ...
  if (ZitLong(s->file, &udata) != _ZIP_OK)
    err = _ZIP_ERRNO;
  else if ((err == _ZIP_OK) && (udata != s->currentfileinfo.crc) &&
           ((uflags & 8) == 0))
    err = _ZIP_BAD_FILE;

  if (ZitLong(s->file, &udata) != _ZIP_OK)
    err = _ZIP_ERRNO;
  else if ((err == _ZIP_OK) && (udata != s->currentfileinfo.compressedsize) &&
           ((uflags & 8) == 0))
    err = _ZIP_BAD_FILE;

  if (ZitLong(s->file, &udata) != _ZIP_OK)
    err = _ZIP_ERRNO;
  else if ((err == _ZIP_OK) && (udata != s->currentfileinfo.uncompressedsize) &&
           ((uflags & 8) == 0))
    err = _ZIP_BAD_FILE;
  ...
  if (ZitShort(s->file, &filenamesize) != _ZIP_OK)
    err = _ZIP_ERRNO;
           ...
           (filenamesize != s->currentfileinfo.filenamesize))
    err = _ZIP_BAD_FILE;
...
static int ZipOpenCurrentFile(Zip *file, const char *password) {
  ...
  if ((s->currentfileinfo.compressionmethod != 0) &&
      (s->currentfileinfo.compressionmethod != Z_DEFLATED))
    err = _ZIP_BAD_FILE;
  ...
  pfileinzipreadinfo->restreaduncompressed =
      s->currentfileinfo.uncompressedsize;

  pfileinzipreadinfo->posinzip = s->currentfileinfointernal.currentfileoffset +
                                 _ZIP_LOCALHEADER_SIZE + isizevar;
  ...
  if (password != NULL) {
    ...
    if (fseek(pfileinzipreadinfo->file,
              s->currentzipfileinfo->posinzip +
                  s->currentzipfileinfo->bytebeforezip,
              SEEK_SET) != 0) {
      FreePatch(pfileinzipreadinfo->buffer);
      FreePatch(pfileinzipreadinfo);
...
static int ZipReadCurrentFile(Zip *file, void *buf, unsigned int len) {
  ...
  if (len > pfileinzipreadinfo->restreaduncompressed)
    pfileinzipreadinfo->stream.avail_out =
        (unsigned int)pfileinzipreadinfo->restreaduncompressed;
  ...
  while (pfileinzipreadinfo->stream.avail_out > 0) {
    if ((pfileinzipreadinfo->stream.avail_in == 0) &&
      ...
      if (s->encrypted) {
        ...
        for (i = 0; i < ureadthis; i++)
          pfileinzipreadinfo->buffer[i] =
              zdecode(s->keys, s->crc32tab, pfileinzipreadinfo->buffer[i]);
      }
    ...
    if (pfileinzipreadinfo->compressionmethod == 0) {
      ...
      for (i = 0; i < udocopy; i++)
        *(pfileinzipreadinfo->stream.next_out + i) =
            *(pfileinzipreadinfo->stream.next_in + i);

      pfileinzipreadinfo->crc32 =
          crc32(pfileinzipreadinfo->crc32, pfileinzipreadinfo->stream.next_out,
                udocopy);
      ...
      pfileinzipreadinfo->crc32 =
          crc32(pfileinzipreadinfo->crc32, bufbefore, (unsigned int)(uoutthis));
...
int ZipExtractCurrentFile(Zip *zip, int *nopath, const char *password) {
  ...
  err = ZitCurrentFileInfo(zip, &fileInfo, filenameinzip, sizeof(filenameinzip),
                           NULL, 0, NULL, 0);
    ...
    do {
      ...
      if (err > 0) {
        ...
        if (remainingSize == 0) {
          ...
          extractBuffer = extractBuffer + err;
      }

    } while (err > 0);

    if (remainingSize != buffersize)
      fwrite(buffer, 1, buffersize - remainingSize, fout);
    fclose(fout);
...
ZipFile *ZipFileRead(Zip *zip, const char *filename, const char *password) {
  ...
  while (err > 0) {
    err = ZipReadCurrentFile(zip, &zipfile->data[count],
