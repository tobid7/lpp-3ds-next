...
Bitmap* LoadBitmap(char* fname){
	...
	if (strncmp("romfs:/",fname,7) == 0){
		...
		FSUSER_OpenFileDirectly(&tmp.handle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_CREATE | FS_OPEN_WRITE, 0x00000000);
	}
	FS_GetSize(&tmp, &size);
...
Bitmap* loadPng(const char* filename)
{
	...
	if (strncmp("romfs:/",filename,7) == 0){
		...
		FSUSER_OpenFileDirectly(&fileHandle.handle, ARCHIVE_SDMC, filePath, filePath, FS_OPEN_READ, 0x00000000);
	}
