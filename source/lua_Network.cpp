...
static int lua_connect(lua_State *L)
{
    ...
    {
        if (connect(my_socket->sock, resaddr_cur->ai_addr, resaddr_cur->ai_addrlen) == 0)
            break;
    }
...
static int lua_addCert(lua_State *L)
{
    ...
    else
    {
        ...
        FS_Archive sdmcArchive;
        FSUSER_OpenArchive(&sdmcArchive, ARCHIVE_SDMC, m_path);
        Result ret = FSUSER_OpenFileDirectly(&fileHandle.handle, a_id, m_path, m_path, FS_OPEN_CREATE | FS_OPEN_WRITE, 0x00000000);
#ifndef SKIP_ERROR_HANDLING
        if (ret)
            return luaL_error(L, "file doesn't exist.");
#endif
