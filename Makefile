...
TARGET		:=	lpp-3ds-next
BUILD		:=	build
SOURCES		:=	source source/include source/include/ftp source/include/ttf source/include/lodepng
DATA		:=	datas
INCLUDES	:=	source source/include source/include/ftp source/include/ttf source/include/lodepng
GRAPHICS	:=	gfx
#GFXBUILD	:=	$(BUILD)
ROMFS		:=	romfs
GFXBUILD	:=	$(ROMFS)/gfx
APP_AUTHOR	:=	Tobi-D7
APP_DESCRIPTION := Updated Lua Player 3ds
ICON		:=	app/icon.png
...
LIBS	:= -llua -lcurl -lstdc++ -lm -lz -lcitro2d -lcitro3d -lctru -ljpeg
